# Modelos de Arquitetura de Software

---

## PaaS (Platform as a Service)
- Plataforma que fornece ambiente e ferramentas para desenvolver, testar e publicar aplicações sem gerenciar servidores.
- Exemplo: Google App Engine.

---

## IaaS (Infrastructure as a Service)
- Fornece infraestrutura de TI, como servidores, redes e armazenamento pela internet/nuvem.
- Exemplo: Microsoft Azure Virtual Machines.

---

## SaaS (Software as a Service)
- Software disponibilizado pela internet que pode ser utilizado diretamente no navegador sem necessidade de instalação.
- Exemplo: Google Docs.

---

## FaaS (Function as a Service)
- Modelo serverless onde pequenas funções de código são executadasem resposta a eventos.
- Exemplo: Azure Functions.

---

## DaaS (Desktop as a Service)
- Serviço que fornece desktops virtuais hospedados em servidores acessíveis remotamente por diferentes dispositivos.
- Exemplo: Citrix DaaS.

---

## DBaaS (Database as a Service)
- Serviço de banco de dados em nuvem onde o provedor gerencia infraestrutura, manutenção e backups.
- Exemplo: MongoDB Atlas.

---

## SECaaS (Security as a Service)
- Modelo que fornece serviços de segurança como proteção contra ataques, autenticação e monitoramento.
- Exemplo: Cloudflare.








# Glossário Aila 5 


---

## Assunto
> Tipos de Sistemas de Informação

---

## Tipos de Sistema de Informação

- **Sistemas de Processamento de Transações (SPT / TPS)**
- **Sistemas de Informação Gerencial (SIG / MIS)**
- **Sistemas de Apoio à Decisão (SAD / DSS)**
- **Sistemas de Apoio Executivo (SAE / EIS)**
- **Sistemas Especialistas / Gestão do Conhecimento (SE / KMS)**

---

## O que cada um faz

### 🔹 Sistemas de Processamento de Transações (SPT)
Cuida das tarefas do dia a dia de uma empresa, como registrar vendas, compras e pagamentose etc.
É a base de dados para todos os outros sistemas.

### 🔹 Sistemas de Informação Gerencial (SIG)
Pega os dados do SPT e transforma em relatórios e resumos para acompanharem como a empresa está indo.

### 🔹 Sistemas de Apoio à Decisão (SAD)
Junta dados internos e externos para ajudar a tomar decisões em situações mais complicadas, onde não tem uma resposta.

### 🔹 Sistemas de Apoio Executivo (SAE / EIS)
Voltado para diretores e presidentes, mostra uma visão geral da organização com gráficos e indicadores para decisões estratégicas.

### 🔹 Sistemas Especialistas / Gestão do Conhecimento (SE / KMS)
Tenta imitar o raciocínio humano usando inteligência artificial e lógicas para resolver problemas em áreas específicas.

---

## Que tipo de SI eles são?

### 🌐 gov.br
> **Sistema de Informação Governamental / Portal de Serviços Públicos (e-Gov)**

É uma plataforma que centraliza serviços públicos digitais num só lugar. Combina SPT, SIG e funciona como um sistema de e-Government, facilitando o acesso da população ao governo.

---

### 🎬 Netflix
> **SPT + SAD + Big Data & Inteligência Artificial + Sistema de Streaming em Nuvem**

| Camada | Função |
|---|---|
| **SPT (TPS)** | Registra assinaturas, pagamentos e acessos dos usuários |
| **SAD (DSS)** | Analisa o comportamento do usuário para sugerir conteúdos |
| **Big Data & IA** | Algoritmos que personalizam as recomendações pra cada pessoa |
| **Streaming em Nuvem** | Distribui os vídeos em tempo real pelo mundo todo |

---

### 📅 Minha Agenda UFN
> **SPT + Sistema de Informação Acadêmico (SIA)**

| Camada | Função |
|---|---|
| **SPT (TPS)** | Registra agendamentos, presenças e atividades dos alunos |
| **SIA** | Gerencia tudo que é específico do ambiente acadêmico, como horários e disciplinas |

---

### 🧾 Sistema de Imposto de Renda do Governo Brasileiro (IRPF)
> **SPT + Big Data & Análise de Dados**

| Camada | Função |
|---|---|
| **SPT (TPS)** | Processa as declarações, pagamentos e restituições dos contribuintes |
| **Big Data & Análise** | Cruza informações de várias fontes para identificar inconsistências e possíveis fraudes |
